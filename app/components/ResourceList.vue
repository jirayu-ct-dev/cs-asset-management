          ...
          <tbody>
            <tr v-for="row in items" :key="row.id">
              ...
              <td v-if="hasActions"><div class="flex flex-wrap gap-1">
                <NuxtLink v-if="endpoint === '/api/inspections'" :to="`/inspections/${row.id}`" class="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3.5 py-2 font-bold border-slate-300 bg-white text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"><UIcon name="i-lucide-clipboard-check" class="size-4" />เปิดรอบ</NuxtLink>
                <template v-if="endpoint === '/api/loans' && row.status === 'ACTIVE'"><button class="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3.5 py-2 font-bold border-transparent bg-teal-700 text-white hover:bg-teal-800 dark:bg-teal-600 dark:hover:bg-teal-500" @click="runAction(row, 'return')"><UIcon name="i-lucide-rotate-ccw" class="size-4" />รับคืน</button><button class="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3.5 py-2 font-bold border-slate-300 bg-white text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800" @click="runAction(row, 'cancel')"><UIcon name="i-lucide-x" class="size-4" />ยกเลิก</button></template>
                <template v-if="endpoint === '/api/repairs'"><button v-if="row.status === 'REPORTED'" class="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3.5 py-2 font-bold border-transparent bg-teal-700 text-white hover:bg-teal-800 dark:bg-teal-600 dark:hover:bg-teal-500" @click="runAction(row, 'send')"><UIcon name="i-lucide-send" class="size-4" />ส่งซ่อม</button><button v-if="row.status === 'SENT'" class="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3.5 py-2 font-bold border-transparent bg-teal-700 text-white hover:bg-teal-800 dark:bg-teal-600 dark:hover:bg-teal-500" @click="runAction(row, 'close-repair')"><UIcon name="i-lucide-package-check" class="size-4" />รับกลับ</button></template>
                <template v-if="endpoint === '/api/disposals' && row.status === 'PROPOSED'"><button class="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3.5 py-2 font-bold border-transparent bg-teal-700 text-white hover:bg-teal-800 dark:bg-teal-600 dark:hover:bg-teal-500" @click="runAction(row, 'complete-disposal')"><UIcon name="i-lucide-trash-2" class="size-4" />จำหน่ายแล้ว</button><button class="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3.5 py-2 font-bold border-slate-300 bg-white text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800" @click="runAction(row, 'reverse')"><UIcon name="i-lucide-undo-2" class="size-4" />ย้อนกลับ</button></template>
                <label class="inline-flex min-h-10 cursor-pointer items-center gap-2 rounded-lg border border-slate-300 px-3 py-2 font-bold dark:border-slate-700"><UIcon name="i-lucide-paperclip" class="size-4" />แนบหลักฐาน<input class="hidden" type="file" @change="uploadAttachment($event, row)"></label>
                <a v-for="file in row.attachments || []" :key="file.id" :href="`/api/attachments/${file.id}`" class="inline-flex min-h-10 items-center gap-2 rounded-lg px-2 font-bold text-teal-700 hover:underline dark:text-teal-400"><UIcon name="i-lucide-download" class="size-4" />{{ file.originalName || file.filename || 'ดาวน์โหลดไฟล์' }}</a>
              </div></td>
            </tr>
          </tbody>
        </table>
