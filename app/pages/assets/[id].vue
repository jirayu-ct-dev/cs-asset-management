...
<template>
  <PageHeader :title="asset?.name || 'รายละเอียดครุภัณฑ์'" :description="asset?.assetNumber">
    <button class="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3.5 py-2 font-bold border-slate-300 bg-white text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800" type="button" @click="printQr">▣ พิมพ์ QR</button><NuxtLink :to="`/assets/${id}/edit`" class="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3.5 py-2 font-bold border-transparent bg-teal-700 text-white hover:bg-teal-800 dark:bg-teal-600 dark:hover:bg-teal-500">แก้ไข</NuxtLink>
  </PageHeader>
  <AppState :status="status" :error="error" @retry="refresh">
    <div class="grid grid-cols-1 gap-4 xl:grid-cols-[1.6fr_.8fr]">
      ...
      <aside class="grid content-start gap-4"><section class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"><div class="flex items-center justify-between gap-4 border-b border-slate-200 p-5 dark:border-slate-800"><h2>QR Code</h2></div><div class="p-5"><div class="grid min-h-44 place-items-center rounded-xl border border-dashed border-slate-300 bg-slate-50 text-center text-slate-500 dark:border-slate-700 dark:bg-slate-950"><img v-if="asset?.id" :src="`/api/assets/${asset.id}/qr`" :alt="`QR ${asset.assetNumber}`" class="max-w-44"><span v-else>กำลังเตรียม QR Code</span></div></div></section><section class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"><div class="flex items-center justify-between gap-4 border-b border-slate-200 p-5 dark:border-slate-800"><h2>งานด่วน</h2></div><div class="grid gap-2 p-5"><button class="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 px-3 py-2 font-bold text-slate-700 dark:border-slate-700 dark:text-slate-200" @click="quickAction(asset?.custodyStatus === 'BORROWED' ? 'loan-state' : 'loan')">{{ asset?.custodyStatus === 'BORROWED' ? 'รับคืน / ดูสถานะยืม' : 'บันทึกการยืม' }}</button><button class="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 px-3 py-2 font-bold text-slate-700 dark:border-slate-700 dark:text-slate-200" @click="quickAction('repair')">แจ้งชำรุด / ซ่อม</button><button class="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 px-3 py-2 font-bold text-slate-700 dark:border-slate-700 dark:text-slate-200" @click="quickAction('transfer')">ย้ายสถานที่</button><button class="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 px-3 py-2 font-bold text-slate-700 dark:border-slate-700 dark:text-slate-200" @click="quickAction('disposal')">เสนอจำหน่าย</button><button class="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 px-3 py-2 font-bold text-slate-700 dark:border-slate-700 dark:text-slate-200" @click="quickAction('inspection')">ตรวจนับ</button></div></section><section class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"><div class="border-b border-slate-200 p-5 font-bold dark:border-slate-800">ไฟล์แนบ</div><div class="grid gap-3 p-5"><input type="file" @change="uploadAttachment"><a v-for="file in asset?.attachments" :key="file.id" :href="`/api/attachments/${file.id}`" class="text-sm font-bold text-teal-700 hover:underline">{{ file.originalName }}</a><p v-if="!asset?.attachments?.length" class="text-sm text-slate-500">ยังไม่มีไฟล์แนบ</p></div></section></aside>
    </div>
  </AppState>
</template>
