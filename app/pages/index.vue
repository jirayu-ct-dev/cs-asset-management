...
<template>
  <PageHeader title="ภาพรวม" description="สถานะทะเบียนและงานที่ต้องดำเนินการล่าสุด">
    <NuxtLink to="/assets/create" class="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3.5 py-2 font-bold border-transparent bg-teal-700 text-white hover:bg-teal-800 dark:bg-teal-600 dark:hover:bg-teal-500">＋ เพิ่มครุภัณฑ์</NuxtLink>
  </PageHeader>
  <AppState :status="status" :error="error" @retry="refresh">
    ...
    <div class="grid grid-cols-1 gap-4 xl:grid-cols-[1.6fr_.8fr]">
      <section class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"><div class="flex items-center justify-between gap-4 border-b border-slate-200 p-5 dark:border-slate-800"><h2>งานที่ต้องดำเนินการ</h2><NuxtLink to="/loans" class="font-bold text-teal-700 hover:underline dark:text-teal-400">ดูทั้งหมด →</NuxtLink></div>
