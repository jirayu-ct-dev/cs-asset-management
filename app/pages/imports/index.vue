...
<template>
  ...
  <section class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 p-5">
    ...
    <template v-else><div class="flex min-h-44 items-center justify-center gap-3 p-8 text-center text-slate-500"><span class="grid size-10 place-items-center rounded-full bg-slate-100 text-xl font-extrabold dark:bg-slate-800">✓</span><div><strong>นำเข้าข้อมูลเสร็จแล้ว</strong><p>สำเร็จ {{ batch?.summary?.imported || 0 }} · ข้าม {{ batch?.summary?.skipped || 0 }} · ผิดพลาด {{ batch?.summary?.invalid || 0 }}</p><NuxtLink to="/assets" class="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3.5 py-2 font-bold border-transparent bg-teal-700 text-white hover:bg-teal-800 dark:bg-teal-600 dark:hover:bg-teal-500">ดูทะเบียนครุภัณฑ์</NuxtLink></div></div></template>
  </section>
</template>
