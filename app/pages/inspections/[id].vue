...
<template><div><PageHeader :title="round?.name || 'รอบตรวจนับ'" :description="round ? `ปีงบประมาณ ${round.fiscalYear}` : ''"><button class="inline-flex min-h-10 items-center rounded-lg bg-red-50 px-4 font-bold text-red-700" @click="closeRound">{{ closing ? 'กำลังปิดรอบ…' : 'ปิดรอบตรวจ' }}</button></PageHeader><AppState :status="status" :error="error" @retry="refresh"><div class="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4"><article v-for="entry in [['ทั้งหมด', summary.total], ['ตรวจแล้ว', summary.inspected], ['ยังไม่ตรวจ', summary.pending], ['ความคืบหน้า', `${progress}%`]]" :key="entry[0]" class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900"><span class="text-sm text-slate-500">{{ entry[0] }}</span><strong class="mt-2 block text-3xl">{{ entry[1] }}</strong></article></div><section class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"><div class="flex gap-2 border-b border-slate-200 p-5 dark:border-slate-800"><input v-model="query" placeholder="สแกน QR หรือค้นหาหมายเลข…"></div><div class="overflow-x-auto"><table><thead><tr><th>หมายเลข/รายการ</th><th>ผลตรวจ</th><th>สถานที่จริง</th><th>สภาพ</th><th>หมายเหตุ</th><th>ดำเนินการ</th></tr></thead><tbody><tr v-for="item in items" :key="item.id"><td><strong>{{ item.snapshotAssetNumber }}</strong><br><span class="text-slate-500">{{ item.snapshotName }}</span></td><td><select v-model="draftFor(item).result"><option value="">เลือกผลตรวจ</option><option value="FOUND_OK">พบ ใช้งานได้</option><option value="FOUND_DAMAGED">พบ ชำรุด</option><option value="REPAIR_REQUESTED">ขอซ่อม</option><option value="MISSING">ไม่พบ</option><option value="DISPOSAL_REQUESTED">ขอจำหน่าย</option><option value="OTHER">อื่น ๆ</option></select></td><td><select v-model="draftFor(item).actualLocationId"><option value="">ไม่ระบุ</option><option v-for="location in references?.locations" :key="location.id" :value="location.id">{{ location.name }}</option></select></td><td><select v-model="draftFor(item).observedCondition"><option value="">ไม่ระบุ</option><option value="NORMAL">ปกติ</option><option value="DAMAGED_USABLE">ชำรุดแต่ใช้ได้</option><option value="UNUSABLE">ใช้งานไม่ได้</option></select></td><td><input v-model="draftFor(item).notes" placeholder="หมายเหตุ"></td><td><button class="rounded-lg bg-teal-700 px-3 py-2 font-bold text-white" @click="saveResult(item)">บันทึกผล</button></td></tr></tbody></table></div></section></AppState></div></template>
